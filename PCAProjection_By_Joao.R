                          ...
                          OutputFolder=NA)
{
  ...
  write.table(SumPCAMat, 'CompImpFile.csv', row.names=RowNames, col.names=ColNames, sep = ",")
  ### variance explained by each component
...
PredictOnNewData <- function(pcaSummary=NA,ProjectonStackFiles=NA,OutputFolder=NA)
{
  ...
  FutStack <- stack(ProjectonStackFiles)
  ...
  for (i in 1:length(FutStack@layers)) 
  {
    ...
    # r2 = rasterize(XYTbl,r1,field=tbl1[,3])
    ## Till here
    # FileName = paste("Comp_Fut",i,".grd")
    # writeRaster(r2,FileName)
    FileName = paste(OutputFolder, "\\Comp",i,".grd", sep = "")
    writeRaster(r2,FileName, overwrite = TRUE)
  }
}
